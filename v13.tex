...
\newcommand{\maybeincludegraphics}[2][]{%
  \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
    \fbox{\parbox[c][3cm][c]{0.9\linewidth}{\centering Add figure: \texttt{\detokenize{#2}}}}}}
...
\noindent \textbf{H1 (Hotel)}: On frequency-encoded tabular features with limited concept drift, a linear margin suffices; a Linear SVM with moderate $C$ will match or beat RBF-SVM, and a time-aware split will not materially change metrics relative to random stratified CV.\\[2pt]
\noindent \textbf{H2 (Accidents)}: Non-linear interactions dominate; a regularized Decision Tree will outperform linear margins, RBF-SVM, kNN, and a compact MLP under the compute budget; probability calibration will improve thresholded F1 at the selected operating point.
...
\paragraph*{Hypothesis verdict (H1).} \emph{Supported.} Relative to random CV, a time-aware split yields small deltas (PR-AUC/ROC-AUC \(\approx\) 0.01--0.02; see split rationale below). Linear SVM outperforms RBF-SVM in PR-AUC (0.8446 vs 0.4796) and shows smoother complexity curves; very large \(C\)/\(\gamma\) increase variance without test gains. Frequency encoding likely induces near-linear separability, explaining the result.
...
% RBF SVM model-complexity (Hotel)
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/hotel/complexity_curve_clf__C_rbf_svm_C.png}}
\caption{\datasetA{}: Model-complexity (RBF SVM: $C$ sweep at fixed $\gamma=\texttt{scale}$); metric: macro PR-AUC (Stratified 3-fold CV).}
\end{figure}

% RBF SVM gamma-sweep (Hotel)
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/hotel/complexity_curve_clf__gamma_rbf_svm_gamma.png}}
\caption{\datasetA{}: Model-complexity (RBF SVM: $\gamma$ sweep at fixed $C$ = best from $C$-sweep); metric: macro PR-AUC (Stratified 3-fold CV).}
\end{figure}

\noindent The $\gamma$ sweep reveals an interior optimum; very small values underfit and very large values overfit, consistent with kernel bandwidth effects.

\noindent The RBF SVM exhibits a clear interior optimum in validation score as $C$ varies at fixed $\gamma$; very small $C$ underfits, while very large $C$ increases variance with no generalization gain, which aligns with the learning-curve diagnosis.
...
\textbf{Hotel: time-aware validation (split rationale).} Because cancellation behavior is tied to arrival dates and seasonality, we sanity-checked a time-aware scheme in addition to the random, stratified split used in the main results. Concretely, we sorted by arrival date (\texttt{arrival\_date\_year}, \texttt{arrival\_date\_month}) and evaluated a hold-out where the last 20\% by time served as validation/test while training on earlier periods (seeds fixed). Model ranking and calibration curves were consistent with the random split; absolute PR-AUC/ROC-AUC differences were small (on the order of 0.01--0.02) and within expected seasonal variation. Given the negligible drift and the goal of sharing identical pipelines across models, we report the random, stratified split in the main results and note that the time-aware check yielded similar conclusions.
...
\paragraph*{Hypothesis verdict (H2).} \emph{Supported.} The calibrated DT leads in PR-AUC (0.7508) and F1 (0.6903) with favorable learning curves. Linear SVM/SGD and kNN trail, and RBF-SVM underperforms at tractable training sizes. Reliability and Brier score indicate useful probability calibration, improving thresholded F1 at the validation-selected operating point. High-dimensional, heterogeneous features with interaction effects favor DT splits under the budget.
...
\centering
\begin{minipage}{0.49\linewidth}
\maybeincludegraphics[width=\linewidth]{figures/accidents/complexity_curve_clf__max_depth_decision_tree_max_depth.png}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\maybeincludegraphics[width=\linewidth]{figures/accidents/complexity_curve_clf__n_neighbors_knn_n_neighbors.png}
\end{minipage}
\vspace{2pt}
\begin{minipage}{0.49\linewidth}
\maybeincludegraphics[width=\linewidth]{figures/accidents/complexity_curve_clf__alpha_linear_svm_sgd_alpha.png}
\end{minipage}
\begin{minipage}{0.49\linewidth}
\maybeincludegraphics[width=\linewidth]{figures/accidents/complexity_curve_clf__alpha_nn_alpha.png}
\end{minipage}
\caption{\datasetB{}: Model-complexity sweeps — (a) DT: max depth, (b) kNN: $k$, (c) Linear SVM/SGD: $\alpha$, (d) NN: L2 $\alpha$.}
\end{figure}

% Linear SVM model-complexity (Accidents) — C sweep
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/accidents/complexity_curve_clf__C_linear_svm_C.png}}
\caption{\datasetB{}: Model-complexity (Linear SVM: $C$ sweep); metric: macro PR-AUC (Stratified 3-fold CV).}
\end{figure}

% RBF SVM model-complexity (Accidents) — γ sweep
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/accidents/complexity_curve_clf__gamma_rbf_svm_gamma.png}}
\caption{\datasetB{}: Model-complexity (RBF SVM: $\gamma$ sweep at fixed $C$ = best from $C$-sweep); metric: macro PR-AUC (Stratified 3-fold CV).}
\end{figure}

\noindent Linear SVM shows a monotone improvement that plateaus as $C$ increases, indicating diminishing returns once the margin penalty is sufficiently relaxed (bias–variance trade-off). The RBF SVM’s $\gamma$ sweep displays an interior optimum: small $\gamma$ underfits, whereas large $\gamma$ causes high variance.

\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/accidents/complexity_curve_clf__C_rbf_svm_C.png}}
\caption{\datasetB{}: Model-complexity (RBF SVM: $C$ sweep at fixed $\gamma=\texttt{scale}$); metric: macro PR-AUC (Stratified 3-fold CV).}
\end{figure}
