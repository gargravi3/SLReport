...
We use a single held-out test set once at the end; hyperparameters are tuned via CV exclusively on the training split (StratifiedShuffleSplit, 3 splits). All seeds are fixed to \seed{}; encoders and scalers are fit within folds to avoid leakage. For classification, we calibrate probabilities and select thresholds by maximizing validation F1. Post-hoc calibration improves probability reliability for threshold selection (cf. \cite{Kull2019DirichletCal}). For Hotel (cancellation exhibits seasonality tied to arrival dates), we use a stratified random split for the main results and include a lightweight month-blocked (year–month) validation check as a robustness study; conclusions remain qualitatively similar, so we report the random split in the main tables. Calibration: we use Platt (sigmoid) via CalibratedClassifierCV with cv=\texttt{prefit}, fitting the calibrator on a 20% slice of the training split; GridSearchCV refits the base model on the full training split at the chosen hyperparameters prior to calibration; we evaluate once on the held-out test.

\section{Methods}
Preprocessing is shared: frequency encoding for categoricals and StandardScaler for numerics in a scikit-learn Pipeline, with float32 features throughout. Labels are stored as int64; features and weights are float32 only. Decision Trees use Gini with regularization (max depth, min samples, ccp\_alpha) and class\_weight=balanced. KNN uses brute-force Euclidean distance with tuned $k$ and weights; we emphasize prediction cost scaling. Prediction is batched (Accidents: chunks of 5k). SVMs include linear (liblinear on Hotel; SGD hinge on Accidents) and RBF kernels, both calibrated post-hoc. NNs are compact MLPs with ReLU activation trained with pure SGD (momentum=0, no Nesterov/Adam), early stopping (patience 3), batch size 1024, and L2 in [1e-4, 1e-3]. We compare shallow–wide (512,512) and deeper–narrower (512,256,128,128) shapes with similar parameter counts ($\sim$0.23–0.28M), keeping epochs $\leq$15. All MLPs stay within the 0.2M–1.0M parameter cap, train for at most 15 epochs with early stopping marked on epoch curves, use batch size 1024, and run in float32 throughout.
...
PR-AUC prevalence baseline: 0.3704.
...
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/hotel/nn_capacity_depth_vs_width.png}}
\caption{\datasetA{}: NN capacity distribution at matched ~0.25--0.30M params (deeper–narrower vs. shallower–wider) — validation metric vs. depth/width.}
\end{figure}

At matched parameter counts, the deeper–narrower shape was slightly more data-hungry (learning curves shift right) while the shallow–wide model trained marginally faster; both remained within the runtime budget.
...
\caption{\datasetA{}: Model-complexity (RBF SVM: C).}
\end{figure}
...
Time-aware validation (Hotel). As a robustness check, we also evaluated a year–month blocked validation; trends match the random split.

\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/hotel/time_block_validation.png}}
\caption{Hotel: time-blocked validation check.}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/accidents/nn_capacity_depth_vs_width.png}}
\caption{\datasetB{}: NN capacity distribution at matched ~0.25--0.30M params (deeper–narrower vs. shallower–wider) — validation metric vs. depth/width.}
\end{figure}

On the large dataset, the deeper–narrower variant benefitted more from additional data (higher peak recall), while shallow–wide was more stable under stronger L2; overall differences were modest at the given budget.
...
Spatiotemporal grouping (Accidents). To assess leakage risk under random splits, we ran a lightweight time-blocked validation on contiguous month blocks; expected deltas appeared but model rankings were unchanged.

\begin{figure}[htbp]
\centerline{\maybeincludegraphics[width=0.95\linewidth]{figures/accidents/time_block_validation.png}}
\caption{Accidents: time-blocked validation check.}
\end{figure}
